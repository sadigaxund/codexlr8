...
3. **OR semantics with token-coverage scoring.** All tokens are searched with OR. A custom scoring layer (path weighting, metadata boosts, match ratio) surfaces files that match more tokens. A post-filter requires ≥50% token match for multi-token queries. Never use pure AND or pure OR without scoring.
...
src/codexlr8/
  ...
  search.py        # FTS5 index, OR query, ranking, status
...
- **Don't revert to AND-only semantics.** OR with scoring is the correct default. If OR returns too many low-quality results, tighten the post-filter threshold or improve the scoring layer, don't weaken the default.
